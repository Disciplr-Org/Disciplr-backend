...
export function verifyDownloadToken(token: string): { jobId: string; userId: string } | null {
  try {
    const { jobId, userId, exp, sig } = JSON.parse(Buffer.from(token, 'base64url').toString()) as {
      jobId: string
      userId: string
      exp: number
      sig: string
