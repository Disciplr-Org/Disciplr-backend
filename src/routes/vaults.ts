...
vaultsRouter.get('/user/:address', authenticate, async (req: Request, res: Response) => {
  try {
    const userVaults = vaults.filter(v => v.creator === req.params.address)
    res.json(userVaults)
  } catch (_err) {
...
vaultsRouter.post('/', authenticate, requireJson, async (req: Request, res: Response) => {
  // Validate with Zod (Soroban-aligned bounds)
 ...
 */
vaultsRouter.post('/:id/cancel', authenticate, requireJson, async (req: Request, res: Response) => {
