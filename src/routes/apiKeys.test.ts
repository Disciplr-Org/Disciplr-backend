...
test("creates, lists, and revokes API keys for an authenticated user", async () => {
  ...
  const revokeResponse = await fetch(
    `${baseUrl}/api/v1/api-keys/${createdBody.apiKeyMeta.id}/revoke`,
    {
      method: "POST",
      headers: {
        "x-user-id": "user-123",
...
test("validates scopes and rejects revoked API keys on protected analytics routes", async () => {
  ...
  const allowedResponse = await fetch(`${baseUrl}/api/v1/analytics/vaults`, {
    headers: {
      "x-api-key": createdBody.apiKey,
    },
  });
  ...
  const revokedResponse = await fetch(`${baseUrl}/api/v1/analytics/vaults`, {
    headers: {
      "x-api-key": createdBody.apiKey,
    },
  });
